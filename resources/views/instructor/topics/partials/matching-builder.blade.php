    ...
    <template x-for="(pair, index) in pairs" :key="pair.id || `pair-${index}`">
        <div class="grid gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 md:grid-cols-[1fr_1fr_auto]">
            <label class="text-xs font-semibold text-gray-700">Left item
                ...
                <input type="text" :name="`configuration[pairs][${index}][left][value]`" x-model="pair.left.value" maxlength="500" required :disabled="activityType !== 'matching'" aria-describedby="activity-configuration-error" class="mt-1 w-full rounded-lg border-gray-300 text-sm">
            </label>
            <label class="text-xs font-semibold text-gray-700">Right item
                ...
                <input type="text" :name="`configuration[pairs][${index}][right][value]`" x-model="pair.right.value" maxlength="500" required :disabled="activityType !== 'matching'" aria-describedby="activity-configuration-error" class="mt-1 w-full rounded-lg border-gray-300 text-sm">
            </label>
